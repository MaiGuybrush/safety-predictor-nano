...
| **②** | **本地模型目錄路徑 (MODEL_PATH)** | 在本地模式下，輸入存放特定模型權重之專屬目錄路徑（如 `models/person_detector/`，請為每個模型建立獨立目錄）。 |
...
2. 於「模型檔案路徑」欄位輸入模型專屬目錄名稱（例：`models/person_detector/`）。
3. 選擇「模型格式偏好類別」為 `「AUTO」` 或指定格式。
4. 點擊 `「[ EXECUTE_UPDATE ]」` 儲存，推論引擎將自動掃描目錄並載入對應格式之模型。
