...
| 標號 | 區塊名稱 | 功能說明 |
|:---:|---|---|
| **①** | **頂部系統狀態列** | 顯示系統運作標題、當前全域模型類別、路徑、CPU 核心數、雲端模型同步狀態按鈕 `「SYNC_MODELS」`、問題回報按鈕 `「[ 回報問題 REPORT ]」`，以及設定面板開關按鈕 `「TOGGLE_CONFIG」`。 |
...
- **`「[ 回報問題 REPORT ]」`**：點擊開啟診斷資料打包與 Gitea Issue 自動回報視窗，支援即時收集日誌並上傳建立問題單（詳見 [第 6 章](./06-system-settings.md#6-問題診斷與異常回報-issue-reporter)）。
