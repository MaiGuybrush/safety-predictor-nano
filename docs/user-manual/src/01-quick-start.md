...
2. **開機網卡自動偵測與廠區選取**：
   - **開箱自動偵測**：設備首次開機或未設定廠區時，系統會自動列舉裝置網卡 IP 並與 UMS API 組態比對網段（如 `10.26.x.x` 自動判定為 `FAB 1`，未命中則預設為 `OA`），並立即自動持久化儲存。
   - **廠區下拉切換**：工程師可於「UMS 服務廠區」下拉選單手動切換所在廠區（如 OA 辦公網段、FAB 1、FAB 2 ...），下方將即時以唯讀清單預覽該廠區的主備雙端點（`src1 (主)` 與 `src2 (備)`），免去手動輸入長網址打錯之風險。
3. 於 **UMS API KEY** 貼上步驟 1 申請的 API Key（可點擊「顯示」確認明文）。
4. 點擊 `「[ 測試連線 ]」` 按鈕，系統將以所選廠區端點執行連線測試，並在介面上清楚回報各端點的獨立連線狀態（如 `src1: 正常 (N models)`）與整體狀態。
