# PRD: 人員裝備防護 (PPE) 檢測與條件式工安合規規則引擎 (PPE Detection & Conditional Compliance Rules)
...
- **REST API (`web_ui.py`)**：
  - `POST /api/external_state`：接收外部系統 Webhook。
  - `GET /api/compliance_status`：取得當前所有串流的設備狀態與工安合規總覽。
- **SSE `/detections_feed` Payload 擴充**：
  - 支援傳遞 `ppe_status`, `violations`, `compliance_summary` 等欄位。
...
1. **單元測試 (`test_compliance_engine.py`, `test_state_poller.py`, `test_config_manager.py`)**。
2. **端對端測試 (`test_compliance_e2e.py`)**：驗證推論、狀態輪詢、規則觸發與快照產出。
