# 串流 FPS 與推檢解耦及動態 Canvas 畫框 Ticket 清單
...
```mermaid
graph TD
    T1["#4: 01-stream-unthrottling-and-clean-video<br/>(StreamHandler & VideoHandler)"]
    T2["#5: 02-per-stream-inference-sampler<br/>(main.py & SSE ts payload)"]
    T3["#6: 03-frontend-canvas-stale-signal<br/>(index.html & Canvas overlay)"]
    T4["#7: 04-e2e-verification-and-syntax-check<br/>(py_compile & E2E)"]
